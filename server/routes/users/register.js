...
// // /register 으로 get 접속하면 views/users/register.pug 를 렌더링해준다.
// // register.pug 는 추후 접속한다.
// router.get('/', function (req, res, next) {
//   res.render('users/register');
// });

// /register 으로 post 접속하면 아래와 같은 동작을 수행한다.
router.post('/', function (req, res, next) {
  ...
  const sql = `INSERT INTO user(USERID, PASSWORD, NAME, SALT) VALUES  ("${user.email}","${user.password}","${user.name}",${user.salt})`;
  ...
  maria.query(sql, function (err, rows) {
    ...
    // 이는 크게 상관없다.
    if (!err) { res.send({ success: "가입되었습니다." }) } else { res.send({ err: "중복된 이메일 입니다." }) };
